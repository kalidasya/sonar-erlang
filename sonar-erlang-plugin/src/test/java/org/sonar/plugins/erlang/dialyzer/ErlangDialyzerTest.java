...
public class ErlangDialyzerTest {
	...
	public void setup() throws URISyntaxException, IOException {
		...
		when(configuration.getString(ErlangPlugin.DIALYZER_FILENAME_KEY, ErlangPlugin.DIALYZER_DEFAULT_FILENAME)).thenReturn(ErlangPlugin.DIALYZER_DEFAULT_FILENAME);
		...
		new DialyzerReportParser()
				.dialyzer(project,context, new ErlangRuleManager(DialyzerRuleRepository.DIALYZER_PATH), rp);
	}
	...
	public void checkDialyzer() {
		...
		assertThat("violation is not D019", capturedViolations.get(0).getRule().getKey(),
				Matchers.equalTo("D019"));
		assertThat("violation is not D041", capturedViolations.get(1).getRule().getKey(),
				Matchers.equalTo("D041"));
		assertThat("violation is not D019", capturedViolations.get(2).getRule().getKey(),
				Matchers.equalTo("D019"));
	}
}
